...
```{r nimble-logistic, cache = TRUE, dependson=-1}
...
```{r nimble-poisson, cahce = TRUE, dependson=-1}
...
```{r stan-poisson-spatial, message = FALSE}
...
```{r stan-poisson-spatial-results, cache = TRUE}
## only plot the regression parameters
print(fit, probs = c(0.1, 0.9), pars = c("beta", "tau2", "phi", "lp__"))
...
```{r stan-poisson-spatial-ppc, cache = TRUE}
p1 <- ppc_intervals(y, y_rep, x = X[, 2])
p2 <- ppc_dens_overlay(y, y_rep)
p3 <- ppc_ecdf_overlay(y, y_rep)  
p1 / p2 / p3  
...
```{r stan-poisson-nospatial, cache = TRUE, message = FALSE}
fit <- stan(
...
```{r stan-poisson-nospatial-results, cache = TRUE}
## only plot the regression parameters
print(fit, probs = c(0.1, 0.9), pars = c("beta", "lp__"))
...
```{r stan-poisson-nospatial-ppc, cache = TRUE}
p1 <- ppc_intervals(y, y_rep, x = X[, 2])
p2 <- ppc_dens_overlay(y, y_rep)
p3 <- ppc_ecdf_overlay(y, y_rep)   
p1 / p2 / p3   
...
```{r spatial-poisson-nimble, cache = TRUE}
...
```{r spatial-poisson-nimble-config, cache = TRUE}
