...
```{r stan-spatial-logistic, message = FALSE}
...
```{r stan-spatial-logistic-results, cache = TRUE}
## only plot the regression parameters
...
```{r stan-spatial-logistic-ppc, cache = TRUE}
p1 <- ppc_intervals(y, y_rep, x = X[, 2])
p2 <- ppc_dens_overlay(y, y_rep)
p3 <- ppc_ecdf_overlay(y, y_rep)
p1 / p2 / p3 
...
```{r stan-nonspatial-fit, cache = TRUE, message = FALSE}
...
```{r stan-nonspatial-fit-results, cache = TRUE}
## only plot the regression parameters
print(fit, probs = c(0.1, 0.9), pars = c("beta", "lp__"))
...
```{r stan-nonspatial-fit-ppc, cache = TRUE}
p1 <- ppc_intervals(y, y_rep, x = X[, 2])  
p2 <- ppc_dens_overlay(y, y_rep)  
p3 <- ppc_ecdf_overlay(y, y_rep)  
p1 / p2 / p3
